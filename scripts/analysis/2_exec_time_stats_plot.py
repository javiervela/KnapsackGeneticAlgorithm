...
TEMP_PLOT_FILENAME = "execution_time_mean_std_temp.png"
AGGREGATED_PLOT_FILENAME_TEMPLATE = "aggregated_execution_time_mean_std_{}.png"
...
for evaluations in FUNCTION_EVALUATIONS:
    ...
    for problem_index in PROBLEM_INDEXES:
        ...
        ax.set_ylabel("Execution Time (Mean ± Std Dev) [ms]")
        ax.grid(True)
        ax.set_title(
            f"Mean Execution Time ± Std Dev for Problem {problem_index} ({evaluations} Evaluations)"
        )
        ax.legend(
    ...
    fig.suptitle(
        f"Execution Time Mean ± Std Dev for {evaluations} Evaluations", fontsize=20
    )
